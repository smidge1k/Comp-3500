...
typedef struct FreeMemoryHoleTag{
  Memory  AdressFirstElement; //Address of first element
...
MemPolicy MemoryPolicy = INFINITE; //chooses which memory policy to use
int PageSize = 8192;
int numAvailablePages = floor(AvailableMemory/PageSize);
...
void LongtermScheduler(void){
  ...
  while (currentProcess) {
    int enoughMem = isEnoughMemory();
      ...
      else { //puts process back in queue if there's not enough memory
...
int isEnoughMemory(int ProcessControlBlock) {
  int enough;
  switch (MemoryPolicy) {
...
int OptimalMemoryAccessPolicy(ProcessControlBlock *currentProcess) {
  if(AvailableMemory >= currentProcess->MemoryRequested) {
    AvailableMemory -= currentProcess->MemoryRequested;
    MemoryAllocated = currentProcess->MemoryRequested;
...
int Paging(ProcessControlBlock *currentProcess) {
  int numRequestedPages = ceiling(currentProcess->MemoryRequested/PageSize);

  if(numAvailablePages >= numRequestedPages) {
    numAvailablePages -= numRequestedPages;
    return 1;
  }
  else {
    return 0;
...
int BestFit (ProcessControlBlock *currentProcess) {
return 0;
}
